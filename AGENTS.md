...
- `mlflow-tests/images/test-run.sh` continues to probe `/mlflow/health` on this branch, but it now runs `mlflow-tests/images/collect-debug-logs.sh` if pre-pytest readiness checks such as `status.url` or `/health` time out
