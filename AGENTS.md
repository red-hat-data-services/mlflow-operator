...
`config/base/networkpolicy.yaml` protects the operator pod: health probes remain reachable on TCP 8081, while metrics TCP 8443 is restricted to the supported platform monitoring namespaces. Keep the explicit `kubernetes.io/metadata.name` namespace selectors aligned with the DSP operator pattern.
...
Keep shipped install manifests under `config/`. CI/local-test-only kustomize trees such as Kind/dev overlays and self-managed PostgreSQL/SeaweedFS scaffolding now live under `.github/test-infra/` so downstream manifest consumers like `opendatahub-operator` only vendor production assets from `config/`. The Kind overlay must re-apply operator image and env replacements from its own `params.env` (`MLFLOW_URL`, `MLFLOW_IMAGE`, `GATEWAY_NAME`, `SECTION_TITLE`) because `config/base` replacements bake those values before the overlay ConfigMap replace; Kind direct artifact-server tests rely on that so `--mlflow-url https://localhost:8444` becomes the generated default artifact root.
...
└── templates/                    # Kubernetes manifests
    ...
    ├── artifacts-deployment.yaml # Optional metadata-aware artifact server
    ├── artifacts-service.yaml    # Optional artifact Service
...
`spec.artifactsServer.enabled` creates an independently scalable `mlflow-artifacts` Deployment and Service; `spec.artifactsServer.replicas` and `spec.artifactsServer.workers` control its pod and per-pod uvicorn worker counts. The operator also creates a `/mlflow-artifacts` `HTTPRoute`, configures the tracking server with `--no-serve-artifacts`, configures both metadata-connected servers with the generated dedicated-route default artifact root, and publishes its complete artifact API root in `status.artifactsUrl`. Because the operator owns that generated root, `spec.defaultArtifactRoot` must be omitted in split mode. That status field is omitted when split serving or its public route is unavailable. The route includes more-specific compatibility matches that rewrite the complete legacy tracking-relative `/mlflow[/suffix]/{api,ajax-api}/2.0/mlflow-artifacts` proxy families to the dedicated prefix, preserving persisted `mlflow-artifacts:/` experiment and run locations, including multipart and presigned operations, when split serving is enabled. It also rewrites the explicit tracking-relative UI artifact handlers for run/model-version downloads, artifact list/upload, trace artifacts, and logged-model artifacts; traffic outside those compatibility rules remains on tracking. The parameterized logged-model endpoints use the narrowest portable Gateway API prefix ending in `/logged-models/`, which necessarily also routes non-artifact logged-model APIs to the metadata-aware artifact Deployment. The tracking Deployment does not mount `spec.storage` in split mode; a file-backed destination is mounted only by the artifact Deployment. As a short-term compatibility topology, the artifact pod runs with `--serve-artifacts`, `--enable-workspaces`, and `--workspace-store-uri=kubernetes://`, receives the same primary/registry/read-replica metadata URI wiring as tracking, and explicitly disables server-side job execution. When metrics are enabled, only the tracking Service carries the `app.kubernetes.io/component: tracking-server` scrape label selected by the `ServiceMonitor`; both Services retain the shared `app` cache label. Operator-managed migrations scale both metadata-connected Deployments to zero and wait for both live replica counts to reach zero before creating the migration Job; disabling split mode during a requested migration defers artifact cleanup until quiescence, migration, and the tracking rollout complete. CA bundle environment settings are rendered through one shared helper for tracking, artifacts, GC, and trace archival, including HTTP/S3 plus PostgreSQL and MySQL clients. Artifact DRA claims are configured independently through `spec.artifactsServer.resourceClaims` and `spec.artifactsServer.resources.claims`; tracking claims are never inherited, while ordinary requests and limits still are. `spec.temporaryStorage.sizeLimit` configures the writable `/tmp` `emptyDir` for both tracking and artifact-server pods. The split mode requires `spec.artifactsDestination`, an explicitly configured external `MLFLOW_URL`, the Gateway API `HTTPRoute` resource, and remote SQL metadata stores. When the `HTTPRoute` API is unavailable, reconciliation fails before cleanup, migration, rendering, or operand mutation so enabling split serving cannot disrupt the current tracking deployment. When split serving is disabled outside migration, cleanup uses the uncached API reader if startup discovery omitted `HTTPRoute` from the cache, ensuring a route from an earlier split deployment is not stranded after a transient discovery failure. Inline SQLite metadata URIs are rejected. CEL cannot inspect Secret-backed metadata URIs, so split-mode reconciliation resolves every configured backend, registry, and read-replica Secret key through the uncached API reader and rejects missing values or non-PostgreSQL/MySQL URIs before cleanup, migration, rendering, or operand mutation. For a `file://` destination, one artifact replica may use `ReadWriteOnce`, while multiple replicas require `ReadWriteMany` as the first access mode. `serveArtifacts` and `artifactsServer.enabled` are mutually exclusive.
The standalone chart defaults `artifactsServer.allowedHosts` to `["*"]` so Gateway Host headers are accepted, matching operator-managed deployments. Production users should replace the wildcard with their external Gateway hostname or hostnames.
...
Kind `test/e2e` has no DSC. `e2e_test.go` covers the `MLflowOperator` handoff in a nested Ordered context: operand Deployment/Service(/HTTPRoute) health, in-cluster Service reachability via `server-info` (HTTPRoute stays object-exists because Kind has no Gateway controller), `MLflow` spec reconcile into the managed Deployment (via `spec.resources` patch and rollback; replica scale-up is not tested on sqlite/RWO Kind because of #190 CEL), `MLflowOperator.status.observedGeneration` after a `spec.gateway.domain` patch, `status.releases` plus the `odh-mlflowoperator-config` platform handshake (including a `platformVersion` bump and restore), and operand GC after `MLflow` deletion. `upgrade_e2e_test.go` asserts `MLflowOperator.status.releases` after a successful 3.10.1 → current migration once the module-controller path is enabled, then deletes MLflow before MLflowOperator so the protection finalizer can release. DSC `Removed` and full platform upgrade/downgrade stay in ODH e2e.
...
Trace archival e2e is split across layers. Ginkgo in `test/e2e/` covers CEL rejection plus operator resource lifecycle and disable-path cleanup; it uses a non-firing CronJob schedule (`0 0 29 2 *`) and dummy postgres/S3 URIs, and it does not start a live archival Job. Disable-path cleanup waits for the CronJob, ConfigMap, and ServiceAccount to disappear, then also waits until ClusterRoleBinding `mlflow` no longer includes `mlflow-trace-archival-sa` and the Deployment no longer references the archival ConfigMap (env, volume, and volumeMount). Live Jobs against `file://` are avoided because the default PVC is ReadWriteOnce and cannot be mounted by a CronJob while the server is running. The test deployer also omits trace archival whenever either metadata store is SQLite because its Kind PVC is `ReadWriteOnce`; those S3 rows test tracking and artifact behavior without the archival CronJob. For S3-compatible storage with PostgreSQL backend and registry stores, `mlflow-tests` smoke coverage creates several traces, waits past a short harness retention (`TRACE_ARCHIVAL_RETENTION`, default `1m` in `test-run.sh`), runs a Job from the CronJob `jobTemplate`, then checks that archive objects appeared, traces remain readable, and `SPANS_LOCATION=ARCHIVE_REPO`. Direct S3 archive checks read `MLFLOW_S3_ENDPOINT_URL`; `test-run.sh` exports that from the SeaweedFS port-forward for `s3` and from the resolved `S3_ENDPOINT_URL`/`AWS_DEFAULT_ENDPOINT` for `externals3`. That live Job smoke is `TestTraceArchival` in `mlflow-tests/tests/test_trace_archival.py` and uses the shared `TestBase` actions/validations step style. Operator deployments serve REST under `--static-prefix=/mlflow`, but the runtime mounts OTLP span ingest at unprefixed `/v1/traces`. OpenShift HTTPRoute rewrites `/mlflow/v1` to `/v1`; Kind port-forward does not. The smoke test posts the OTLP payload to both the prefixed tracking URI and the unprefixed origin so the archival scheduler sees `SPANS_LOCATION=TRACKING_STORE`.

Garbage-collection e2e follows the same split. Ginkgo verifies `spec.garbageCollection` creates and, when removed, cleans up `mlflow-gc`, `mlflow-gc-sa`, and the suffixed GC ClusterRole/ClusterRoleBinding; that role must grant `list` on cluster-scoped `namespaces` because `mlflow gc --all-workspaces` resolves the Kubernetes workspace provider. It uses the same non-firing schedule. For S3-compatible rows with PostgreSQL backend and registry stores, the harness enables GC and `TestGarbageCollection` soft-deletes an experiment containing a run artifact, starts a one-off Job from the `mlflow-gc` CronJob template, and verifies that the run, experiment, and backing S3 object are permanently removed. This test is also selected in the split artifact-server S3 matrix, where it covers artifact-server locations. Do not enable the live GC smoke test for `file://` rows with the default ReadWriteOnce PVC.

For self-deployed SeaweedFS TLS runs, `mlflow-tests/images/test-run.sh` extracts all `.crt` and `.pem` entries from the configured CA ConfigMap and exports them through the standard Python, requests, curl, and AWS CA environment variables. Preserve that trust setup for presigned artifact downloads; do not replace it with disabled certificate verification.
...
- Normal current-version multi-backend `mlflow-tests/images/test-run.sh` runs must tear down the `MLflow` CR plus any self-managed PostgreSQL / SeaweedFS infrastructure between backend suites so the next suite does not inherit metadata or object-storage state from the prior one. The same cluster-scoped `MLflow` CR must also be deleted after the last suite, including pytest failures, and `test-run.sh` must trap `INT`/`TERM` as well as `EXIT`. A leftover instance keeps `MLflowOperator` from reaching Removed (`MLflowInstancesPresent` / `mlflow.opendatahub.io/mlflow-operator-protection`). Do not rely on Jenkins stage teardown to delete it.
...
- Launch host-networked upgrade test containers through `mlflow-tests/images/run-integration-tests.sh`; it maps `minio-service.<namespace>.svc.cluster.local` to the harness port-forward for SeaweedFS presigned URLs and is the shared source of container launch behavior
...
- With `SKIP_DEPLOYMENT=true`, resolve `spec.artifactsServer.enabled` and `spec.serveArtifacts` from the reused MLflow CR before artifact validation, readiness, URI setup, and pytest exports. Omitted CR booleans default to false; read failures must emit harness JUnit errors. Do not overwrite reused settings with deployment flags or validate unused metadata deployment flags. `ARTIFACTS_SERVER_GATEWAY` is a requested validation mode, effective only when the CR enables the artifact server. Fresh deployments remain flag-driven.
...
- Dedicated artifact-server smoke coverage is split by backend and capability. The operator GitHub Kind workflow installs the pinned `HTTPRoute` CRD before operator startup, provides `mlflow-artifacts-tls`, and runs independent PostgreSQL/PostgreSQL/file and PostgreSQL/PostgreSQL/S3 rows. Both port-forward the artifact Service to test workspace-authenticated upload, list, and download directly; multipart create/abort runs only for S3-compatible storage. Downstream OpenShift additionally sets `ARTIFACTS_SERVER_GATEWAY=true` to wait for route acceptance and verify tracking-relative rewrites from artifact pod access logs. All tests carry the `smoke` and `artifacts_server` markers; Kind skips only the Gateway-specific tests.
...
7. **mlflow_v1_mlflow_artifacts_server.yaml** - Dedicated artifact server
   - Separate tracking and metadata-aware artifact-serving Deployments
   - Independent artifact-server replicas and resources
   - Commented workload-specific Dynamic Resource Allocation claims
