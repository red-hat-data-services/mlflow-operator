...
Kind `test/e2e` has no DSC. `e2e_test.go` covers the `MLflowOperator` handoff in a nested Ordered context: operand Deployment/Service(/HTTPRoute) health, in-cluster Service reachability via `server-info` (HTTPRoute stays object-exists because Kind has no Gateway controller), `MLflow` spec reconcile into the managed Deployment (via `spec.resources` patch and rollback; replica scale-up is not tested on sqlite/RWO Kind because of #190 CEL), `MLflowOperator.status.observedGeneration` after a `spec.gateway.domain` patch, `status.releases` plus the `odh-mlflowoperator-config` platform handshake, and operand GC after `MLflow` deletion. `upgrade_e2e_test.go` asserts `MLflowOperator.status.releases` after a successful 3.10.1 → current migration once the module-controller path is enabled, then deletes MLflow before MLflowOperator so the protection finalizer can release. DSC `Removed` and full platform upgrade/downgrade stay in ODH e2e.
