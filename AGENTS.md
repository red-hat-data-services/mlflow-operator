...
└── mlflowconfig/
    └── v1/
        ├── groupversion_info.go     # MLflowConfig API group and version registration
        ├── mlflowconfig_types.go    # MLflowConfig resource type definitions
        └── zz_generated.deepcopy.go # Auto-generated DeepCopy methods
```
...
3. The CRDs will be updated at:
   ...
   - `config/crd/bases/mlflow.kubeflow.org_mlflowconfigs.yaml`
...
- API fields are added, removed, or renamed in `api/v1/mlflow_types.go` or `api/mlflowconfig/v1/mlflowconfig_types.go`
...
1. All samples are valid against the CRD:
   ...
   #   kubectl apply -f config/crd/bases/mlflow.kubeflow.org_mlflowconfigs.yaml
